...
$\limsup\limits_{k\rightarrow\infty}(|\frac{s_{k+1}-s}{s_{k}-s}|)$  mit den Reihengliedern $s_k = \frac{1}{k^2}$, bzw $s_{k+1}= \frac{1}{(k+1)^2}$  und dem bekannten Grenzwert $s = \frac{\pi^2}{6}$ \footnote{J.-P. Serre: A course in arithmetic, Springer-Verlag New York (1973), ISBN 978-0-387-90040-7, S. 91.}.
...
Es fällt auf, dass das naive Summenverfahren deutlich länger als die beiden Implementationen nach Borwein braucht, um gute Ergebnisse zu liefern.
