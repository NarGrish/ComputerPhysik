...
Das naive Verfahren konvergiert nur sehr langsam. Da wir es hier nur nutzen um später die ersten 100 Schritte grafisch mit den anderen Verfahren vergleichen zu können, laufen wir an dieser Steller auch nur über diese ersten 100 Summenglieder, um unnötige Wartezeiten zu vermeiden.
```{Rcpp}
#include <Rcpp.h>
...
Rcpp::List borweinTestEuler(int s){
  ...
  // Quelltext
    ...
    summe /= (1-pow(2,1-s));
    Rprintf("Das Verfahren nach Bohrwein nach Gauss verbessert ergibt %.16lf\n", summe);
    // Rückgabe für eine grafische Wiedergabe
    return List::create(Named("x") = x, Named("y") = y);
    }
```
